document.addEventListener("DOMContentLoaded", () => {
  ...
  const closeAssignOverlayButton = document.getElementById(
    "close-assign-overlay-button"
  );
    ...
    badges.forEach((badge) => {
      ...
      function showContextMenu(x, y) {
        ...
        assignOption.addEventListener("click", () => {
          ...
          const assignBadgeOverlay = document.getElementById(
            "assign-badge-overlay"
          );
        ...
        deleteOption.addEventListener("click", () => {
          ...
          const confirmed = confirm(
            `Do you really want to DELETE the Badge "${badge.name}" ?`
          );
          if (confirmed) {
...
document
  .getElementById("assign-badge-form")
  .addEventListener("submit", async (event) => {
    event.preventDefault();
    ...
    try {
      ...
      const selectedBadge = badgeDetails.find(
...
async function fetchAndDisplayBadgeUsers(badgeName) {
  try {
    ...
    const userData = await response.json();
    const badgeUsers = userData.users.filter(
      (user) => user.badges && user.badges.some((b) => b.name === badgeName)
    ); // Add a check for user.badges
...
async function fetchBadgeUsers(badgeName) {
  try {
    ...
    const userData = await response.json();
    const badgeUsers = userData.users.filter((user) =>
...
async function deleteBadge(badgeName) {
  try {
    ...
    const updatedUsers = userData.users.map((user) => {
      const updatedBadges = user.badges.filter(
        (badge) => badge.name !== badgeName
      );
    ...
    console.log(
      `Das Badge "${badgeName}" wurde erfolgreich von den Benutzern entfernt.`
    );
