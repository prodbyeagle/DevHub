  ...
  async function renderBlogPosts() {
    ...
    const authorInfo = `${profilePictureHTML} ${post.author}`;
      ...
      if (post.image) {
        ...
        const imageContainer = postElement.querySelector(
      ...
      fixedEmojis.forEach((emoji) => {
        ...
        emojiElement.addEventListener("click", async () => {
          try {
            ...
            const userReactions =
              JSON.parse(
                localStorage.getItem(`reactions_${postId}_${userIdentifier}`)
              ) || {};

            if (!userReactions[emoji]) {
              // Benutzerreaktion hinzufügen
              const reactionAdded = await addReactionToPost(
                postId,
                emoji,
                userIdentifier
              );
              ...
              const reactionRemoved = await removeReactionFromPost(
                postId,
                emoji,
                userIdentifier
              );
            ...
            localStorage.setItem(
              `reactions_${postId}_${userIdentifier}`,
...
async function getUserReactions(postId, userIdentifier) {
  try {
    const response = await fetch(
      `/api/blogs/${postId}/userReactions/${userIdentifier}`
    );
    if (!response.ok) {
